...
function SongData({ state, dispatch }: SideNavProps): JSX.Element {
  ...
  const currentnotes: any = state.get('notes');
  console.log('now current notes', state.get('notes'));

  const currentsong: any = songs.find(
    (s: any) => s.get('notes') === currentnotes
  );
  console.log('curSong', currentsong);

  console.log('Here current song', currentsong);
  console.log('Here songs', currentsong?._root.entries[3][1]);
  const curArtist = currentsong?._root.entries[3][1];
  const curAlbum = currentsong?._root.entries[4][1];
  const curDifficulty = currentsong?._root.entries[5][1];

  //console.log('currentsong data', currentsong.get('size'));
  return (
    <Section title='Song Data'>
      {
        <div>
          <div className='mr1' />
          <p>
  ...
  return (
    <div className='absolute top-0 left-0 bottom-0 w5 z-1 shadow-1 bg-white flex flex-column'>
      ...
      <div className='flex-auto'>
        ...
        <SongData state={state} dispatch={dispatch} />
      </div>
    </div>
  );
}
